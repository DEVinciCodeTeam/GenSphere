...
signUpForm.onsubmit = function(e) {
  ...
  // Insertar if de verificacion (si es el primer ususario o no)
  // Verificar si alguien ya de esa corte o no (en caso de que no crear la clave en allUsers)
...
signInForm.onsubmit = function(e) {
  ...
  const usersNum = Object.keys(allUsers.users).length;

  while (index != usersNum && allUsers.users[index].userEmail != getUserEmail) {
    console.log(allUsers.users[index].userEmail != getUserEmail);
    index++;
  }
  ...
  if (getUserCohorte == identifiedUser.userCohorte && getUserPassword == identifiedUser.userPassword) {
    ...
    if (getUserCohorte != identifiedUser.userCohorte) {
      alert("La cohorte no coincide con el correo que proporcionaste");
    } else if (getUserPassword != identifiedUser.userPassword) {
